...
- `getVenueImage` is the single source of truth for card images and now prefers stable ID paths if a local URL looks out of sync.
...
- `validate_image_sync.py` reports one local URL mismatch (`Dorp Street`), which indicates data drift is still possible.
...
2. **Avoid preloading everything by default** — preloading 349 images up front is expensive for mid-tier devices and cellular data. A staged strategy (first 10–20 + adjacent cards) balances speed and memory.
...
- Fix the single `image_url` mismatch flagged by validation (run `python scripts/migrate_images_to_stable_ids.py --apply`).
